...
Cb=[1 2;1 5;2 3;2 4;3 5;4 5];
Cs=[1 3;1 4;3 4;2 5;2 7;2 8;2 6];
% q=qu=1,2,3
% alpha=0.5;
% ID_PERT_COEF = 0.01;
% TRIAL_NUM = 60;
...
% Ck=[eye(POS_NUM) zeros(VEL_NUM,VEL_NUM) ];%eye(SYS_NUM);%[eye(12) zeros(12,54) ];%
% q=qu=1
% ID_PERT_COEF = 0.01;
% TRIAL_NUM = 150;
% ITE_NUM = 20;
...
%             u_new(:,i) = u_norm(:,i) + du_seq(IN_NUM*(STEP_NUM-i+1)+1:IN_NUM*(STEP_NUM-i+2),1);
            % control limit
            du_seq(IN_NUM*(STEP_NUM-i+1)+1:IN_NUM*(STEP_NUM-i+2),1)=min(0-u_norm(:,i),-K(:,:,i)*[dz_seq(OUT_NUM*(STEP_NUM-i+1)+1:OUT_NUM*(STEP_NUM-i+1+q),1);du_seq(IN_NUM*(STEP_NUM-i+2)+1:IN_NUM*(STEP_NUM-i+1+qu),1)] + alpha*kt(:,i));  
            u_new(:,i) = u_norm(:,i) + du_seq(IN_NUM*(STEP_NUM-i+1)+1:IN_NUM*(STEP_NUM-i+2),1);
...
for k=1:1:STEP_DIV
    for j=1:1:TRIAL_NUM
        ...
        for i=ID_START(k):1:ID_END(k)
            % control limit
            delta_u(IN_NUM*(STEP_NUM-i+1)+1:IN_NUM*(STEP_NUM-i+2),j)=min(0-u_norm(:,i),delta_u(IN_NUM*(STEP_NUM-i+1)+1:IN_NUM*(STEP_NUM-i+2),j));
