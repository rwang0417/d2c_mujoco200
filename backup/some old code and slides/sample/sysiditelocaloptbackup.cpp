...
void check(mjModel* m, mjData* d)
{
	...
	{
		for (h = 0; h < step_max; h++) delta_xc[h][u] = ptb_coef * gaussrand();
	}
...
void setcontrol(mjModel* m, mjData* d)
{
	...
	while (index < step_max)
	{
		...
		{
			delta_x1[i + NS] = ptb_coef * gaussrand();
		...
		for (y = 0; y < NS; y++)
		{
			...
			{
				mat[index][y][i] = mat[index][y][i] * (1 - 1 / (trial + 1.0)) + delta_x2[y] * delta_x1[i] / ((trial + 1.0) * ptb_coef * ptb_coef);
			}
		}
...
int main(int argc, const char** argv)
{
	...
	if ((fop = fopen("../../../data/converge0.7.txt", "wt+")) == NULL) {
		return 0;
	}
	...
	if (STATE_LINEAR == true)
	{
		...
		if ((fop2 = fopen("../../../data/lineart0.006s6p0.7.txt", "wt+")) != NULL)
		{
			str3 = glstr;
			for (int t = 0; t < step_max; t++)
			{
		...
		if ((fp1 = fopen("../../../data/states.txt", "at+")) != NULL)
		{
			str3 = glstr;
			for (int h = 0; h < NS; h++)
			{
