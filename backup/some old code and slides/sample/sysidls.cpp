...
const int32_t step_max = 300;
const int32_t roll_max = 22000;
...
mjtNum delta_x1[step_max][roll_max][NS + ctrl_num] = { 0 };
mjtNum delta_x2[step_max][NS][roll_max] = { 0 };
mjtNum mat[step_max][NS][NS + ctrl_num] = { 0 };
mjtNum sum[step_max][NS][NS + ctrl_num] = { 0 };
mjtNum flags[step_max][NS][NS + ctrl_num] = { 1 };
mjtNum dx[step_max + 1][NS + ctrl_num];
...
void setcontrol(mjtNum time, mjtNum* ctrl, int nu)
{
	if (cal_flag == true)
	{
		if (time - t_init < t_step - 0.0001)
		{
			...
			if (t_init < -0.1)
			{
				...
				if (index >= step_max) {
					...
					for (int y = 0; y < m->nq; y++)
					{
						delta_x1[0][0][2 * y] = ptb_coef * gaussrand();
						d->qpos[y] = delta_x1[0][0][2 * y] + state_nominal[0][2 * y];
					}
					for (int y = 0; y < m->nv; y++)
					{
						delta_x1[0][0][2 * y + 1] = ptb_coef * gaussrand();
						d->qvel[y] = delta_x1[0][0][2 * y + 1] + state_nominal[0][2 * y + 1];
					}
					for (int ci = 0; ci < ctrl_num; ci++)
					{
						delta_x1[0][0][ci + NS] = ptb_coef * gaussrand();
						ctrl[ci] = ctrl_nominal[0 * ctrl_num + ci] + delta_x1[0][0][ci + NS];
					}
				}
				else {
					for (int ci = 0; ci < ctrl_num; ci++)
		...
		if (time - t_init < t_step - 0.0001)
		{
			...
			for (int y = 0; y < m->nq; y++)
			{
				delta_x2[index][2 * y][trial] = d->qpos[y] - state_nominal[index + 1][2 * y];
			}
			for (int y = 0; y < m->nv; y++)
			{
				delta_x2[index][2 * y + 1][trial] = d->qvel[y] - state_nominal[index + 1][2 * y + 1];
			}
			...
			/*for (int r = 0; r < NS; r++)
			{
				...
				{
					buff = mat[index][r][c] * (1 - 1 / (trial + 1.0)) + delta_x2[r] * delta_x1[c] / ((trial + 1.0) * ptb_coef * ptb_coef);
					if (flags[index][r][c] == 1)
					{
						sum[index][r][c] += fabs(buff - mat[index][r][c]);
						if ((trial % 5) == 0)
						{
							if (sum[index][r][c] < 0.0001) flags[index][r][c] = 0;
							sum[index][r][c] = 0;
						}
					}
					mat[index][r][c] = buff;
			...
			for (int y = 0; y < m->nq; y++)
			{
				delta_x1[index][trial][2 * y] = ptb_coef * gaussrand();
				d->qpos[y] = delta_x1[index][trial][2 * y] + state_nominal[index][2 * y];
			}
			for (int y = 0; y < m->nv; y++)
			{
				delta_x1[index][trial][2 * y + 1] = ptb_coef * gaussrand();
				d->qvel[y] = delta_x1[index][trial][2 * y + 1] + state_nominal[index][2 * y + 1];
			}
			for (int ci = 0; ci < ctrl_num; ci++)
			{
				delta_x1[index][trial][ci + NS] = ptb_coef * gaussrand();
				ctrl[ci] = ctrl_nominal[index * ctrl_num + ci] + delta_x1[index][trial][ci + NS];
			}
		}
	}
}
			...
			/*for (int t = 0; t < step_max; t++)
			{
				for (int h = 0; h < NS; h++)
				{
					for (int d = 0; d < NS + ctrl_num; d++)
					{
						if (flags[t][h][d] == 1) {
							sprintf(str3, "%d ", t);
							fwrite(str3, 3, 1, fop2);
							sprintf(str3, "%d ", h);
							fwrite(str3, 2, 1, fop2);
							sprintf(str3, "%d\n", d);
							fwrite(str3, 2, 1, fop2);
						}
					}
