...
// model selection: PENDULUM CARTPOLE CART2POLE CART3POLE FISH
#define FISH
...
mjtNum mat[step_max][NS-1][NS + ctrl_num] = { 0 };
mjtNum sum[step_max][NS-1][NS + ctrl_num] = { 0 };
...
void setcontrol(mjtNum time, mjtNum* ctrl, int nu)
{
	if (cal_flag == true)
	{
				...
				if (index >= step_max) {
					...
					for (int y = 0; y < m->nq; y++)
					{
						if (y < 4) {
							...
							d->qpos[y] = delta_x1[2 * (y - 1)] + state_nominal[0][2 * y];
						}
					}
					if (d->qpos[3] > 1) d->qpos[3] = d->qpos[3] - 2;
		...
		if (time - t_init < t_step - 0.00001)
		{
			...
			for (int y = 0; y < m->nq; y++)
			{
				if (y < 4) delta_x2[2 * y] = d->qpos[y] - state_nominal[index + 1][2 * y];
				else if (y > 4) delta_x2[2 * (y - 1)] = d->qpos[y] - state_nominal[index + 1][2 * y];
			}
			for (int y = 0; y < m->nv; y++)
			{
			...
			if (index >= step_max)
			{
				...
				if (trial >= roll_max)
				{
					...
					for (int y = 0; y < m->nq; y++)
					{
						if (y < 4) d->qpos[y] = delta_xc[index][2 * y] + state_nominal[index][2 * y];
						else if (y > 4) d->qpos[y] = delta_xc[index][2 * (y - 1)] + state_nominal[index][2 * y];
					}
					if (d->qpos[3] > 1) d->qpos[3] = d->qpos[3] - 2;
					d->qpos[4] = sqrt(1 - d->qpos[3] * d->qpos[3] - d->qpos[6] * d->qpos[6] - d->qpos[5] * d->qpos[5]);
					for (int y = 0; y < m->nv; y++)
					...
					while (index < step_max - 1)
					{
						if (d->time - t_init < t_step - 0.00001)
						{
							...
							for (int y = 0; y < m->nq; y++)
							{
								if (y < 4) delta_xcs[index][2 * y] = d->qpos[y] - state_nominal[index + 1][2 * y];
								else if (y > 4) delta_xcs[index][2 * (y - 1)] = d->qpos[y] - state_nominal[index + 1][2 * y];
							}
							for (int y = 0; y < m->nv; y++)
							{
			...
			for (int y = 0; y < m->nq; y++)
			{
				if (y < 4) {
					...
					delta_x1[2 * (y - 1)] = ptb_coef * gaussrand();
					d->qpos[y] = delta_x1[2 * (y - 1)] + state_nominal[index][2 * y];
				}
			}
			if (d->qpos[3] > 1) d->qpos[3] = d->qpos[3] - 2;
			d->qpos[4] = sqrt(1 - d->qpos[3] * d->qpos[3] - d->qpos[6] * d->qpos[6] - d->qpos[5] * d->qpos[5]);
...
int main(int argc, const char** argv)
{
	...
	if (STATE_LINEAR == true)
	{
		...
		if ((fop2 = fopen("../../../data/linearization.txt", "wt+")) != NULL)
		{
			...
			for (int t = 0; t < step_max; t++)
			{
				for (int h = 0; h < NS - 1; h++)
				{
					for (int d = 0; d < NS + ctrl_num - 1; d++)
					{
