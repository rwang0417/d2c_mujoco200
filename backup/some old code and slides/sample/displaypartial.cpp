...
mjtNum state_nominal[step_max + 1][NS] = { -3*PI/4.0, 0, 0, 0 };
// top is 0
mjtNum x_goal[NS] = {
	(mjtNum)(0),
	(mjtNum)(0),
...
mjtNum TK[step_max][ctrl_num][12] = { 0 };
...
//		}
		...
		if (d->time - t_init < t_step - 0.000001)
		{
			...
			if (t_init < -0.1)
			{
				...
				//state[12] = d->qpos[9] - state_nominal[index][16];
				for (int y = 0; y < 6; y++)
				{
					state[2 * y + 1] = d->qvel[y] - state_nominal[index][2 * y + 1];
					state_closedlp[index][2 * y + 1] = d->qvel[y];
				}
				//state[13] = d->qvel[8] - state_nominal[index][17];
				for (int y = 6; y < m->nv; y++)
				{
					state_closedlp[index][2 * y + 1] = d->qvel[y];
				}
				if (index < step_max) {
					mju_mulMatVec(res0, *TK[index], state, ctrl_num, 12);
					for (int ci = 0; ci < ctrl_num; ci++)
					{
...
void perfcheck()
{
	...
	{
		for (mjtNum ptb = 0; ptb <= 0.20001; ptb += 0.05)
		{
...
void errcompare()
{
	...
	for (double ptb = .05; ptb <= 0.20001; ptb += 0.05)
	{
		k = (int)(20 * ptb);
		strcpy(dfilename, dfilepre);
