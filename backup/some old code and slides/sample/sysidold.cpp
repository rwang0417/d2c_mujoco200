...
const int32_t roll_max = 30000;
const int32_t step_max = 100;
const int ctrl_num = 1;
const int NS = 16;
mjtNum ptb_coef = 0.15;
	...
	if (cal_flag == true)
	{
		if (time - t_init < t_step - 0.0001)
		{
			...
			if (t_init < -0.1)
			{
				...
				{
					state_nominal[0][2 * y] = d->qpos[y];
				}
				for (int y = 0; y < m->nv; y++)
				{
					state_nominal[0][2 * y + 1] = d->qvel[y];
				}
			}
			else {
				index++;
		...
		if (time - t_init < t_step - 0.0001)
		{
			...
			{
				str3 = glstr;
				sprintf(str3, "%3.3f", mat[index][7][14]);
				fwrite(str3, 5, 1, fop);
				fputs(" ", fop);
			}*/
...
int main(int argc, const char** argv)
{
	...
	if ((fop = fopen("../../../data/cost.txt", "wt+")) == NULL) {
		return 0;
	}
