...
def latexplot(timefactor=240,filtered=False):
    #plot preprocessing
#    bmap = brewer2mpl.get_map('Set2','qualitative', 7)
#    colors = bmap.mpl_colors
...
def normcost(timefactor=0.043):
    with open('cost.txt') as f:
        r=f.readlines()
    y=np.array(r[0].strip().split()).astype(np.float)
    x=np.linspace(1,y.shape[0],y.shape[0])*timefactor
    plt.figure(figsize=(12,9))
    plt.vlines(x[-1],0,y[0]/y[-1],colors='r',linestyles='--')
    plt.text(x[-1],1,69800.0)
    b,a=signal.butter(8,0.025)
    plt.plot(x,y/y[-1],color='r',linewidth=3)
    plt.grid(linestyle='--')
    #plt.xlabel('time/s')
...
def perfcheck(nstart=0,nend=100,type='error',noisemax=100):
    if type=='cost':
        ...
        plt.figure(figsize=(12,9))
        f5,=plt.plot(np.arange(sind,(eind-1)*step+1,step),cost[sind:eind],'orange')
...
def noiseresist(nstart=0,nend=100):
    ...
    eind=int(nend/10)+1
    plt.figure(figsize=(12,9))
    f5,=plt.plot(np.arange(sind,eind*10,10),cost[sind:eind]/cost[0],'orange')
    plt.fill_between(np.arange(sind,eind*10,10),(cost[sind:eind]-cstd[sind:eind])/cost[0],(cost[sind:eind]+cstd[sind:eind])/cost[0],alpha=0.3,color='orange')
    plt.xlabel('noise/%')
...
def perfcompare(filename='errorbar0.3.txt'):
    y=np.transpose(np.loadtxt(filename))
    plt.figure(figsize=(15,12))
    f1,=plt.plot(y[4][:],y[0][:],'orange')
    f2,=plt.plot(y[4][:],y[2][:],'dodgerblue')
    plt.fill_between(y[4][:],y[0][:]-y[1][:],y[0][:]+y[1][:],alpha=0.3,color='orange')
    plt.fill_between(y[4][:],y[2][:]-y[3][:],y[2][:]+y[3][:],alpha=0.3,color='dodgerblue')
    plt.xlabel('noise/%')
    plt.ylabel('error')
    plt.legend(handles=[f1,f2,],labels=['closed-loop error','open-loop error'],loc='upper left')
    plt.grid(True)
    plt.show()
    
def finalcost(nstart=0,nend=100):
    y=np.array(np.loadtxt('finalcost.txt'))
    cost=np.mean(y,axis=1)
    cstd=np.std(y,axis=1)
    sind=int(nstart/10)
    eind=int(nend/10)+1
    plt.figure(figsize=(12,9))
    f5,=plt.plot(np.arange(sind,eind*10,10),cost[sind:eind],'orange')
    plt.fill_between(np.arange(sind,eind*10,10),cost[sind:eind]-cstd[sind:eind],cost[sind:eind]+cstd[sind:eind],alpha=0.3,color='orange')
    plt.xlabel('noise/%')
