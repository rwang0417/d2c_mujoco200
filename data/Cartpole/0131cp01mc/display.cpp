...
const mjtNum t_step = 0.1;
const mjtNum ptb_coef = 0.4;
const int step_max = 25;
const int ctrl_num = 1;
const int NS = 4;
const mjtNum K[NS] = { -1.0959, -2.3829, -33.7244, -8.8701 };// { -1.0959, -2.3829, -33.7244, -8.8701 }; //{ -0.0787, - 0.1996, - 13.0807 ,- 3.0313 };
...
int setcontrol(mjModel* m, mjData* d)
{
	...
	if (TOP_FLAG == true)
	{
		if (d->time - t_init < t_step - 0.000001)
		{
			...
			for (int ci = 0; ci < ctrl_num; ci++)
			{
				d->ctrl[ci] = -res0[ci];// +ptb_coef * uabsmax * gaussrand();
			}
			t_init = d->time;
		}
	}else {
