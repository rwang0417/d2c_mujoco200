...
#timefactor=6.41
# ilqr: timefactor=20.81
def latexplot(timefactor=20.81,filtered=False):
    #plot
    if filtered == True:
...
def clopcompare():
    ...
    with open('clopbar.txt', 'wt+') as f:
        for k in range(pointnum):
            print(np.mean(clerr1[testnum*k:testnum*(k+1)]), np.std(clerr1[testnum*k:testnum*(k+1)]), np.mean(operr1[testnum*k:testnum*(k+1)]), np.std(operr1[testnum*k:testnum*(k+1)]), k*20, file=f)
    ...
    # plt.xlabel('Std dev of process noise(Percent of max. control)')
