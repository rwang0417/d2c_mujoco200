...
# timefactor=4.01
def latexplot(timefactor=400,filtered=True):
    #plot
    if filtered == True:
...
def showcurve(filename='cost.txt', mode = 'c'):
    if mode == 'r':
...
def perfcheck(nstart=0,nend=100,type='error',noisemax=100):
    if type=='cost':
        ...
        plt.ylabel('cost per step',fontsize=18)
        plt.show()  
        print('averaged by {value1} rollouts'.format(value1=y.shape[1]))
    ...
    if type=='error':
        ...
        plt.xlabel('Std dev of perturbed noise (Percent of max. control)',fontsize=18)
        plt.ylabel('L2-norm of terminal state error',fontsize=18)
        plt.show()  
        print('averaged by {value1} rollouts'.format(value1=y.shape[1]))
...
def errcompare(nstart=0,nend=100, pointnum=20, testnum=100, by='state'):
