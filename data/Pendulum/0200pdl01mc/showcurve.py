...
#y=np.loadtxt('cost.txt')
#cm = np.mean(y[:,0:1000],axis=0)
#cs = np.std(y[:,0:1000],axis=0)
#x=np.linspace(1,1000,1000)*1.1
#plt.plot(x, cm/cm[-1], color='dodgerblue', linewidth=1)
#plt.fill_between(x,(cm-cs)/cm[-1],(cm+cs)/cm[-1],alpha=0.5,color='orange')
#plt.xlabel('Num of rollouts', fontsize=20)
#plt.ylabel('Averaged episodic cost fraction', fontsize=20)
...
def normcost(timefactor=0.026):
    ...
    plt.vlines(y.shape[0]*timefactor,0,y[0]/y[-1],colors='r',linestyle='--')
    plt.text(x[-1], y[-1], 220000, ha='right', va= 'bottom',fontsize=10)
    plt.show()
    print("NUM = {value1}".format(value1=y.shape[0]))
...
def finalcost(nstart=0,nend=100,noisemax=100):
...
def errcompare(nstart=0,nend=100, pointnum=10, testnum=1000, by='state'):
